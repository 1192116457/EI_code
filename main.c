...
*/
	...
	if (!( Gval= doubleMalloc(5000))) SUPERLU_ABORT("Malloc fails for Gval[].");
	if (!( GrowIdx= intMalloc(5000))) SUPERLU_ABORT("Malloc fails for GrowIdx[].");
	if (!( GcolPtr= intMalloc(5000))) SUPERLU_ABORT("Malloc fails for GcolPtr[].");
	...
	read_file_double("/home/jerry/ei/csc_data/example5by5/Gval.txt", Gval, Gval_size);
	read_file_int("/home/jerry/ei/csc_data/example5by5/GrowIdx.txt", GrowIdx, GrowIdx_size);
	read_file_int("/home/jerry/ei/csc_data/example5by5/GcolPtr.txt", GcolPtr, GcolPtr_size);
	
	/* C */
	double *Cval;
	int *CrowIdx, *CcolPtr, *Cval_size, *CrowIdx_size, *CcolPtr_size;
	if (!( Cval= doubleMalloc(5000))) SUPERLU_ABORT("Malloc fails for Cval[].");
	if (!( CrowIdx= intMalloc(5000))) SUPERLU_ABORT("Malloc fails for CrowIdx[].");
	if (!( CcolPtr= intMalloc(5000))) SUPERLU_ABORT("Malloc fails for CcolPtr[].");
	...
	read_file_double("/home/jerry/ei/csc_data/example5by5/Cval.txt", Cval, Cval_size);
	read_file_int("/home/jerry/ei/csc_data/example5by5/CrowIdx.txt", CrowIdx, CrowIdx_size);
	read_file_int("/home/jerry/ei/csc_data/example5by5/CcolPtr.txt", CcolPtr, CcolPtr_size);
	...
	if (!( u_t= doubleMalloc(1000))) SUPERLU_ABORT("Malloc fails for u_t[].");
	if (!( u_th= doubleMalloc(1000))) SUPERLU_ABORT("Malloc fails for u_th[].");
	...
	read_file_double("/home/jerry/ei/csc_data/example5by5/u_t.txt", u_t, u_t_size);
	read_file_double("/home/jerry/ei/csc_data/example5by5/u_th.txt", u_th, u_th_size);

	/* x(t) */
	double * x_t;	
	int *x_t_size;
	if (!(x_t = doubleMalloc(1000))) SUPERLU_ABORT("Malloc fails for x_t[].");
	if (!(x_t_size = intMalloc(1))) SUPERLU_ABORT("Malloc fails for x_t_size[].");

	read_file_double("/home/jerry/ei/csc_data/example5by5/x_t.txt", x_t, x_t_size);
	...
	dCreate_CompCol_Matrix(&C, matrix_dim, matrix_dim, *Cval_size, Cval, CrowIdx, CcolPtr, SLU_NC, SLU_D, SLU_GE);
